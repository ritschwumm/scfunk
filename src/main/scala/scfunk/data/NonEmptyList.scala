...
object NonEmptyList {
	def pure[T](value:T):NonEmptyList[T]	= NonEmptyEnd(value)
	
	// TODO speedup
	def apply[T](value1:T, values:T*):NonEmptyList[T]	=
			option(values) match {
				case Some(values)	=> NonEmptyMid(value1, values)
				case None			=> NonEmptyEnd(value1)
			}
		
	// TODO speedup
	def option[T](value:Seq[T]):Option[NonEmptyList[T]]	= value.size match {
		case 0	=> None
		case 1	=> Some(NonEmptyEnd(value.head))
...
sealed abstract class NonEmptyList[+T] {
	...
	def :+ [U>:T](value:U):NonEmptyList[U]	= cata (
			...
			(value1)		=> value1 :: NonEmptyEnd(value))
	
	/** cons an element to this NonEmptyList. */
	def :: [U>:T](value:U):NonEmptyList[U]	= NonEmptyMid(value, this)
	...
	def map[U](func:T=>U):NonEmptyList[U]	= cata(
			(value, next)	=> NonEmptyMid(func(value), next map func),
			(value)			=> NonEmptyEnd(func(value)))
